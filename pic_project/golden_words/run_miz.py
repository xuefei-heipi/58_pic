...
if __name__ == '__main__':
    ...
    sql_get_word1 = 'SELECT * FROM `58pic_compepitor_words` where `crawled` = 0 and `miz`=0 and `date` = "20200930" and keyword <> "" and id < 9397;'
    sql_get_word2 = 'SELECT * FROM `58pic_compepitor_words` where `crawled` = 0 and `miz`=0 and `date` = "20200930" and keyword <> "" and id BETWEEN 9396 and 11397 ;'
    sql_get_word3 = 'SELECT * FROM `58pic_compepitor_words` where `crawled` = 0 and `miz`=0 and `date` = "20200930" and keyword <> "" and id > 11397;'
    ...
    try:
        # 线程处理
        threads = []
        t1 = threading.Thread(target=run, args=(rows1, logger, nipic, miz, baotu, 2,))
        t2 = threading.Thread(target=run, args=(rows2, logger, nipic, miz, baotu, 2,))
        t3 = threading.Thread(target=run, args=(rows3, logger, nipic, miz, baotu, 2,))
        threads.append(t1)
        threads.append(t2)
        threads.append(t3)
