...
class Hosts(models.Model):
    masterHost = models.CharField(max_length=30, null=False)
    dlHost = models.CharField(max_length=30, null=False)
    uploadHost = models.CharField(max_length=30, null=False)

    class Meta:
        db_table = 'Hosts'

class TestStatus(models.Model):
    fastJsCssStatus = models.IntegerField()     # 0: 未测试 1: 测试中  2：测试完成
    fastConsoleStatus = models.IntegerField()   # 0: 未测试 1: 测试中  2：测试完成
    fastCheckUsername = models.CharField(max_length=30, null=False)     #检测控制台需要的用户名
    fastCheckPassword = models.CharField(max_length=30, null=False)     #检测控制台需要的密码

    class Meta:
        db_table = 'TestStatus'

class JsCssCheckInfo(models.Model):
...
class FastCheckDetail(models.Model):
...
class ConsoleCheckInfo(models.Model):
    startTime = models.DateTimeField()      #url检测的开始时间
    endTime = models.DateTimeField()        #url检测的结束时间
    checkFileNum = models.IntegerField()    #检测的url数量
    errorFileNum = models.IntegerField()    #检测的url出错数量
    checkFile = models.TextField()          #失效

    class Meta:
        db_table = 'ConsoleCheckInfo'


class ConsoleCheckDetail(models.Model):
    errorInfo = models.TextField()         #检测url报错的详细信息

    class Meta:
        db_table = 'ConsoleCheckDetail'


class JSIgnore(models.Model):
    title = models.CharField(max_length=200, null=False)
    remark = models.TextField(null=True)
...
class ConsoleUrl(models.Model):
    url = models.CharField(max_length=200, null=False)              #检测url链接
    urlDescription = models.CharField(max_length=200, null=False)   #检测url链接描述
    remark = models.TextField(null=True)                            #检测url备注

    class Meta:
        db_table = "ConsoleUrl"


class ConsoleErrorIgnore(models.Model):
    errorContent = models.CharField(max_length=200, null=False)     #检测url忽略的报错信息
    remark = models.TextField(null=True)                            #检测url忽略的报错信息备注

    class Meta:
        db_table = "ConsoleErrorIgnore"


class FunctionTest(models.Model):
    functionName = models.CharField(max_length=200, null=False)     #用例名称
    functionStatus = models.IntegerField()                          #用例执行状态
    type = models.IntegerField()                                    #用例标记
    testName = models.CharField(max_length=200, null=True)         #用例文件名称

    class Meta:
        db_table = "FunctionTest"


class FunctionReport(models.Model):
    reportName = models.CharField(max_length=200, null=False)   #报告名称
    type = models.IntegerField()                                #报告标识
